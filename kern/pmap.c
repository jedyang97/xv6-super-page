...
struct PageInfo *super_pages;           // Physical super page state array
...
boot_alloc(uint32_t n)
{
  ...
  if (n > 0) {
    ...
    panic("boot_alloc: n must be greater than or equal to zero!");
  }
...
mem_init_mp(void)
{
  ...
  for (i = 0; i < NCPU; i++) {
    boot_map_region(
            kern_pgdir,
            KSTACKTOP - KSTKSIZE - i * (KSTKGAP + KSTKSIZE),
            KSTKSIZE,
...
page_init(void)
{
  ...
  int free_physical_page_num = (int) ROUNDUP(PADDR(((char *) envs) + (NENV * sizeof(struct Env))), PGSIZE) >> PGSHIFT;
  for (i = free_physical_page_num; i < npages; i++) {
    pages[i].pp_ref = 0;
    pages[i].pp_link = page_free_list;
    page_free_list = &pages[i];
  }
...
page_alloc(int alloc_flags)
{
  ...
  if (page_free_list != NULL) {
    // pop the last page from page_free_list
...
struct PageInfo *
super_page_alloc(int alloc_flags)
{
  if (super_page_free_list != NULL) {
    struct PageInfo *result = super_page_free_list;
    super_page_free_list = super_page_free_list->pp_link;
    if (alloc_flags & ALLOC_ZERO) {
      memset(super_page2kva(result), 0, PTSIZE);
    }
    result->pp_link = NULL;
    return result;
  }
...
page_free(struct PageInfo *pp)
{
  ...
  if (pp->pp_ref != 0) {
    panic("page_free: pp->pp_ref must be zero!");
  }
  // if (pp->pp_link != NULL) {
  //   panic("page_free: pp->pp_link must be NULL!");
  // }
...
void
super_page_free(struct PageInfo *pp)
{
  if (pp->pp_ref != 0) {
    panic("super_page_free: pp->pp_ref must be zero!");
  }
  // if (pp->pp_link != NULL) {
  //   panic("super_page_free: pp->pp_link must be NULL!");
  // }
  pp->pp_link = super_page_free_list;  
  super_page_free_list = pp;
}
...
void
super_page_decref(struct PageInfo* pp)
{
  if (--pp->pp_ref == 0)
    super_page_free(pp);
}
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
  ...
  if (!(pgdir[pd_idx] & PTE_P)) {
    // page table does not exist, create one if needed
    if (create) {
      struct PageInfo *new_pt = page_alloc(ALLOC_ZERO);
      if (new_pt == NULL) { // page_alloc fails
        return NULL;
      }
      new_pt->pp_ref++;
...
pde_t *
super_pgdir_walk(pde_t *pgdir, const void *va, int create)
{
  int pd_idx = PDX(va);
  if (!(pgdir[pd_idx] & PTE_P)) {
    if (create) {
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
  ...
  for (; begin < end; begin += PGSIZE) {
    ...
    if (pte == NULL || begin >= ULIM || !(*pte & PTE_P) || (*pte & perm) != perm ) {
...
benchmark(void) {
  ...
  assert((super_page0_at_va1 = super_page_alloc(1)));
  assert((page1_at_va0 = page_alloc(1)));
  assert((super_page1_at_va1 = super_page_alloc(1)));
  assert((page2_at_va2 = page_alloc(1)));
  assert((super_page2_at_va2 = super_page_alloc(1)));
  ...
  assert(!(page_insert(kern_pgdir, page2_at_va2, va2, PTE_W)));
  assert(!(super_page_insert(kern_pgdir, super_page2_at_va2, va2, PTE_W)));

  assert(!(super_page_insert(kern_pgdir, super_page3_at_va3, va3, PTE_W)));
  assert(!(page_insert(kern_pgdir, page3_at_va3, va3, PTE_W)));
